...
def leastConstraintValue(row, col, num, arr):
  tempArr = deepcopy(arr)
  tempArr[row][col] = num
  weights = []
  zeroArray = getIndex(0,tempArr)
  for eachZero in zeroArray:
    if(len(getDomain(eachZero[0],eachZero[1], initialArray))):
      return -1
    else:
...
def assignNumber(row,col,arr):
  ...
  for eachNumber in domain:
    ...
    if(str(arr) not in allStatesCreated):
      domainNumbers.append(eachNumber)
      LCVArray.append(leastConstraintValue(row, col, eachNumber, initialArray))
      breakVariable = 1
  if(breakVariable == 0):
    if(len(listOfZeroesFilled) != 0):
      ...
      # global checker 
      # checker = 1
  elif(breakVariable == 1):
    chosenNumber = domainNumbers[LCVArray.index(max(LCVArray))]
    initialArray[row][col] = chosenNumber
    tempArr = []
    tempArr.append(row)
    tempArr.append(col)
